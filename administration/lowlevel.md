...
У объекта может быть более одного слоя:
- основной слой. Есть у любых объектов БД.
Это данные объектов, например, табличные страницы, версии строк таблиц, индексные записи, последовательности и др.
Внутри основного слоя для таблиц хранятся табличные страницы, которые попадают в [буферный кеш](wal.md) при работе.
- карта свободного пространства или `free space map`. Она есть у таблиц и индексов.
- карта видимости или `visibility map`. Она есть только у таблиц.
- слой инициализации. Он существует только для нежурналируемых таблиц и их индексов.
...
- `82129` - основной слой
- `82129_fsm` - карта свободного пространства
- `82129_vm` - карта видимости
...
Посмотрим с помощью функции `pg_relation_size(<имя_объекта>, <тип_слоя>)` размер слоев:
```sql
SELECT
...
Вначале слой содержит один единственный файл с именем - числовым идентификатором.
Или несколькими слоями, которые могут иметь окончание, в зависимости от типа слоя, - `_fsm`, `_vm` или `_init`.
Когда размер слоя доходит до `1 Гб`, создается следующий файл этого же слоя.
Эти файлы называются сегментами.
