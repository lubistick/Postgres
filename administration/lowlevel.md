...
- `82129` - основной слой - внутри него хранятся табличные страницы, которые попадают в [буферный кеш](wal.md) при работе
...
Теперь подробнее про типы слоев.

### Основной слой

Слой `main`. Есть у любых объектов БД. Это собственно данные:
- табличные страницы
- версии строк таблиц
- индексные записи
- последовательности и др.

У объекта помимо основного слоя могут быть и дополнительные слои.

### Карта свободного пространства

Слой `fsm` или `free space map`. Есть у таблиц и индексов.

### Карта видимости

Слой `vm` или `visibility map`. Есть только у таблиц.

### Слой инициализации

Слой `init` существует только для нежурналируемых таблиц и их индексов.
