...
Колонки:
- `datname` - имя БД
- `datistemplate` - является ли БД шаблоном
- `datallowconn` - разрешены ли соединения с БД (по умолчанию к `template0` подключения запрещены)
...
Конструкция `$user` обозначает схему с тем же именем,
что и имя текущего пользователя (в нашем случае `postgres`).
...
Объект можно перемещать между схемами.
Поскольку речь идет о логической организации, перемещение происходит только в системном каталоге,
сами данные физически остаются на месте.
```sql
...
Теперь к таблице `t` можно обращаться только с явным указанием схемы:
```sql
SELECT * FROM app.t;
...
Если опустить имя схемы, таблица не будет найдена:
```sql
SELECT * FROM t;
