...
Параметры `random_page_cost` и `seq_page_cost` используются планировщиком запросов
и задают примерную стоимость чтения с диска одной страницы данных при произвольном и последовательном доступе соответственно. 
Чем меньше соотношение между этими параметрами, тем чаще планировщик будет предпочитать [индексный доступ](../optimization/indexscan.md) вместо [последовательного сканирования](../optimization/seqscan.md) таблицы.

Значения по умолчанию параметров `random_page_cost` и `seq_page_cost` больше подходят для медленных HDD-дисков.
...
Параметры `seq_page_cost` и `random_page_cost` также можно установить в [`postgresql.conf`](configuration.md), тогда они будут действовать для всех табличных пространств.
