...
Привилегии БД:
- `CONNECT` - разрешает подключение к этой БД (не путать с атрибутом `LOGIN` у роли, который позволяет подключаться к кластеру `Postgres`)
- `CREATE` разрешает создавать схемы в этой БД
- `TEMPORARY` - разрешает создание временных таблиц (поскольку точное имя схемы для временных объектов заранее неизвестно)
