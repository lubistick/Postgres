...
Общая сложность соединения хешированием пропорциональна сумме числа строк в одном и другом наборах данных.
Метод соединения хешированием гораздо эффективнее вложенного цикла при большом числе строк.
Однако, чтобы начать соединение, требуется заплатить накладные расходы на построение хеш-таблицы.
Поэтому при небольшом числе строк вложенный цикл более эффективен.
...
Как и хеш-индекс, хеш-соединение может работать только с условием равенства - хеш-функция не сохраняет порядок значений.
...
Независимо от того, в каком порядке перечислены таблицы в соединении, хеш-таблица строится по меньшему набору строк.
...
Если строка принадлежит первому пакету, она сопоставляется с хеш-таблицей.
...
`3` файла размером примерно `14 000 000` относится к первой таблице, и `3` размером примерно `19 000 000` ко второй.
...
Узел `Hash Join` и читает `temp read`, и записывает `written` временные файлы на этапе соединения.
...
Сначала соединяются `tickets` (билеты) и `ticket_flights` (перелеты).
Затем получившийся набор строк соединяется с `flights` (перелетами), по которым строится другая хеш-таблица.
