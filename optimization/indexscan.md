...
Мы получаем данные из индекса, спускаясь от корня дерева к левому листовому узлу и проходя по списку листовых страниц.
Поэтому индексное сканирование всегда возвращает данные в том порядке, в котором они хранятся в девере индекса и который был указан при его создании:
```sql
...
(2 rows)
```
План то же самый, только добавилось ключевое слово `Backward`.
...
Обратите внимание на количество страниц (buffers), которые потребовалось прочитать:
```sql
...
Получим тот же результат с помощью конструкции `IN` и посмотрим, сколько страниц потребовалось прочитать в этом случае:
```sql
